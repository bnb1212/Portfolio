...
IMDB 데이터셋과 마찬가지로, 각 뉴스는 (같은 방식을 사용한) 단어 인덱스의 시퀀스로 인코딩되어 있습니다.
================================================='''
...
print(train_data[:2])
print(train_label[:2], ' ', set(train_label))
...
x_test = vector_seq(test_data)  # test_data를 벡터화
print(x_train)
print(x_test)
...
one_hot_train_label = to_onehot(train_label)  # train_label의 원핫 처리
one_hot_test_label = to_onehot(test_label)  # test_label의 원핫 처리
...
model.add(layers.Dense(64, input_shape=(10000,), activation='relu'))
model.add(layers.Dense(46, input_shape=(10000,), activation='softmax'))
print(model.summary())
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])

# 모델 훈련 시 검증 데이터(validation data)를 사용
x_val = x_train[:1000]
...
# 시각화
plt.plot(epochs, loss,'bo', label='train acc')
plt.plot(epochs, val_loss, 'r', label='train val-acc')
plt.xlabel('epochs')
plt.ylabel('acc')
plt.legend()
plt.show()
