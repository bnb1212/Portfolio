...
print(y_train[:5])
y_teat = to_categorical(y_test)

# 이미지 보강 예) 기존 이미지를 좌우대칭, 약간 회전, 기울기, 확대/축소, 평행이동 등의 작업을 통해 다양한 이미지로 모델학습
...
x_augment = img_generate.flow(x_augment, np.zeros(augment_size), batch_size=augment_size,
                              shuffle=False).next()[0]

# 원래 x_train에 Image augment된 x_augment를 추가
x_train = np.concatenate((x_train, x_augment))
y_train = np.concatenate((y_train, y_augment))
print(x_train.shape, ' ', y_train.shape)

model = tf.keras.models.Sequential([  # filter - 노드 갯수
    # layer1
    tf.keras.layers.Conv2D(fileters=32, kernel_size=(3, 3), input_shape=(28, 28, 1), padding='same', activation='relu'),
    tf.keras.layers.MaxPool2D(pool_size=(2, 2)),
    tf.keras.layers.Dropout(rate=0.3),
    
    # layer2
    tf.keras.layers.Conv2D(fileters=64, kernel_size=(3, 3), input_shape=(28, 28, 1), padding='same', activation='relu'),
    tf.keras.layers.MaxPool2D(pool_size=(2, 2)),
    tf.keras.layers.Dropout(rate=0.3),
...
print(f'train acc : {model.evaluate(x_train, y_train)[1]:.4f}')
print(f'train loss : {model.evaluate(x_train, y_train)[0]:.4f}')
